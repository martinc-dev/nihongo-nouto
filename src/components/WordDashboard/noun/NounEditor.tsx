...
interface NounWordWithTags extends NounWord {
  nounTagRel?: NounTagRelItem[]
}

const NounEditor = ({ wordId = null }: NounEditorProps) => {
  ...
  useEffect(() => {
    if (!isCreateMode && word && 'word' in word) {
      ...
      setFormData({
        word: nounWord.word || '',
        hiragana: nounWord.hiragana || '',
        sense: nounWord.sense || '',
      })

      if (nounWord.nounTagRel && Array.isArray(nounWord.nounTagRel)) {
        const tagIds = nounWord.nounTagRel
          .map(rel => rel.tagId)
  ...
  const handleFieldChange = (field: keyof NounWord, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }))
  ...
  return (
    <WordEditorContainer>
      ...
      <div className={editorClasses.formSection}>
        ...
        <Box className={editorClasses.tagsContainer}>
          {Object.values(nounTags).map(tag => {
            const isSelected = selectedTagIds.includes(tag.id)

            return (
              <Chip
                className={editorClasses.tagChip}
